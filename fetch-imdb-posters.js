#!/usr/bin/env node
/**
 * Movie Poster Fetcher - Using IMDb title search
 */
...
const englishTitles = {
    ...
    'AI': 'A.I. Artificial Intelligence 2001',
    ...
    '尚气和十环传奇': 'Shang-Chi and the Legend of the Ten Rings 2021',
    '恐怖游轮': 'Triangle 2009',
    '雨人': 'Rain Man 1988',
    '沙丘': 'Dune 2021',
...
function fetchPoster(title, year) {
    return new Promise((resolve) => {
        const searchTitle = englishTitles[title] || `${title} ${year}`;
            ...
            res.on('end', () => {
                try {
                    ...
                    if (results && results.length > 0 && results[0].i) {
                        ...
                        resolve(`https://m.media-amazon.com/images/M/${imageId}`);
                    } else {
                        resolve(null);
                    }
                } catch (e) {
...
async function fetchAllPosters() {
    ...
    for (let i = 0; i < movies.length; i++) {
        ...
        let posterUrl = null;
        for (let retry = 0; retry < 2 && !posterUrl; retry++) {
            posterUrl = await fetchPoster(movie.title, movie.year);
            if (posterUrl && usedPosters.has(posterUrl)) {
                posterUrl = null;
            }
            if (!posterUrl && retry < 1) {
                await sleep(500);
            }
        }
        ...
        if ((i + 1) % 25 === 0) {
            ...
            console.log(`\n--- Progress: ${updated} updated, ${failed} failed, ${usedPosters.size} unique ---\n`);
        }
    }
    
    fs.writeFileSync(OUTPUT_FILE, JSON.stringify(movies, null, 2));
    
    console.log(`\nDone! ${updated} updated, ${failed} failed, ${usedPosters.size} unique posters`);
}
